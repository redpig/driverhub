...
//
// Standard C library functions (memcpy, strlen, snprintf, etc.) are NOT
// re-declared here because they conflict with C++ system headers that
// add noexcept specifiers. The system headers already provide these
// declarations, and the linker resolves them to glibc.
...
// Kernel-specific string/conversion functions (not in standard libc).
...
// sort/bsearch (kernel versions — may differ from libc).
