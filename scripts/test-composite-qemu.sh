#!/bin/bash
# scripts/test-composite-qemu.sh
#
# End-to-end test: cross-compile the DriverHub Composite Node Demo for
# Fuchsia, build a ZBI, boot in QEMU, and verify all 9 tests pass.
#
# This demo loads TWO .ko modules:
#   1. gpio_controller_module.ko — registers a 32-pin GPIO chip
#   2. touchscreen_composite_module.ko — consumer that requests pins 3 + 8
#
...
#
# Usage:
#   ./scripts/test-composite-qemu.sh          # x86_64 (default)
#   ./scripts/test-composite-qemu.sh arm64     # aarch64
#
# The script exits 0 if all 9 composite demo tests pass in the QEMU serial output.
...
echo "=== DriverHub Composite Node QEMU Test ($ARCH) ==="
echo ""
...
OBJDIR=/tmp/composite-obj-$ARCH
OUTPUT_ZBI=/tmp/fuchsia-composite-$ARCH.zbi
OUTPUT_BIN=/tmp/composite-demo-$ARCH
OUTPUT_GPIO_KO=/tmp/gpio_controller_module-composite-$ARCH.ko
OUTPUT_TS_KO=/tmp/touchscreen_composite_module-$ARCH.ko
QEMU_LOG=/tmp/qemu-composite-output-$ARCH.log
...
# Compile the composite demo binary.
$CXX $CXXFLAGS -c -o "$OBJDIR/composite_demo.o" src/fuchsia/composite_demo.cc 2>&1
echo "  Compiled composite_demo.cc"
...
# ---------------------------------------------------------------------------
# Step 4: Cross-compile the .ko modules
# ---------------------------------------------------------------------------
echo ""
echo "=== Step 4: Cross-compiling .ko modules ==="
...
$CC --target="$TARGET_TRIPLE" -c -o "$OUTPUT_TS_KO" \
  tests/touchscreen_composite_module.c -Isrc/shim/include \
  -fno-stack-protector -fno-pie 2>&1
echo "  Built: $OUTPUT_TS_KO"
echo "  $(file "$OUTPUT_TS_KO")"
...
# The composite demo takes two .ko paths as arguments.
CMDLINE_FILE=$(mktemp)
echo 'zircon.autorun.boot=/boot/bin/composite-demo+/boot/data/modules/gpio_controller.ko+/boot/data/modules/touchscreen.ko' > "$CMDLINE_FILE"

$ZBI_TOOL -o "$OUTPUT_ZBI" \
  "$ORIG_ZBI" \
  --entry bin/composite-demo="$OUTPUT_BIN" \
  --entry data/modules/gpio_controller.ko="$OUTPUT_GPIO_KO" \
  --entry data/modules/touchscreen.ko="$OUTPUT_TS_KO" \
...
if grep -q "Composite Node Demo Results" "$QEMU_LOG"; then
  echo "--- Test output ($ARCH) ---"
  grep -E "composite|driverhub-gpio|dh-touchscreen|GPIO|Chip|Pin|Composite|Results|PASS|FAIL|Module|loaded|probed" "$QEMU_LOG" \
    | sed 's/.*> //' | grep -v "^$"
  echo "---"
  echo ""
  ...
  if [ "$FAIL_COUNT" -eq 0 ] && [ "$PASS_COUNT" -ge 9 ]; then
  ...
  echo "ERROR: Composite Node Demo output not found in QEMU log."
